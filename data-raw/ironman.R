...
ironman <- marathon |>
  ...
  mutate(
    y = case_when(
      Gender == "Female" & `Division Rank` %in% c(1:10) ~ 1,
      Gender == "Male" & `Division Rank` %in% c(1:10) ~ 1,
      .default = 0
    ),
    quotient_model = case_when(
      Gender == "Female" ~ hms(`Overall Time`) / hms("8:10:34"),
      Gender == "Male" ~ hms(`Overall Time`) / hms("7:24:20")
    ),
    y_hat = case_when(
      Gender == "Male" & quotient_model >= 1.041185 & quotient_model <= 1.089985 ~ 1,
